 ...
 */
static BOOL
bCheckBytes(struct fastbuf *pFile, const UCHAR *aucBytes, size_t tBytes)
{
	int	iIndex, iChar;
	...
	for (iIndex = 0; iIndex < (int)tBytes; iIndex++) {
		iChar = bgetc(pFile);
		if (iChar < 0 || iChar != (int)aucBytes[iIndex]) {
			NO_DBG_HEX(iChar);
 ...
 */
BOOL
bIsWordForDosFile(struct fastbuf *pFile, long lFilesize)
{
 ...
 */
static BOOL
bIsWordFileWithOLE(struct fastbuf *pFile, long lFilesize)
{
	static UCHAR	aucBytes[] =
 ...
 */
BOOL
bIsRtfFile(struct fastbuf *pFile)
{
	static UCHAR	aucBytes[] =
		{ '{', '\\', 'r', 't', 'f', '1' };
 ...
 */
BOOL
bIsWordPerfectFile(struct fastbuf *pFile)
{
	static UCHAR	aucBytes[] =
		{ 0xff, 'W', 'P', 'C' };
 ...
 */
BOOL
bIsWinWord12File(struct fastbuf *pFile, long lFilesize)
{
 ...
 */
BOOL
bIsMacWord45File(struct fastbuf *pFile)
{
 ...
 */
int
iGuessVersionNumber(struct fastbuf *pFile, long lFilesize)
{
 ...
 */
int
iInitDocument(struct fastbuf *pFile, long lFilesize)
{
