...
bAdd2TextBlockList(const text_block_type *pTextBlock)
{
	...
	if(pTextBlock->bUsesUnicode && odd(pTextBlock->ulLength)) gerror(2200, "Antiword: Unicode string with odd byte-length found");
 ...
 */
static BOOL
bIsEmptyBox(struct fastbuf *pFile, const list_mem_type *pAnchor)
{
	const list_mem_type	*pCurr;
	size_t	tIndex, tSize;
	UCHAR	*aucBuffer;
 ...
 */
void
vSplitBlockList(struct fastbuf *pFile, ULONG ulTextLen, ULONG ulFootnoteLen,
 ...
 */
static USHORT
usGetNextByte(struct fastbuf *pFile, list_mem_type *pAnchor,
 ...
 */
static USHORT
usGetNextChar(struct fastbuf *pFile, list_mem_type *pAnchor,
	ULONG *pulFileOffset, ULONG *pulCharPos, USHORT *pusPropMod)
{
	USHORT	usLSB, usMSB;
 ...
 */
USHORT
usNextChar(struct fastbuf *pFile, list_id_enum eListID,
	ULONG *pulFileOffset, ULONG *pulCharPos, USHORT *pusPropMod)
{
	USHORT	usRetVal;
