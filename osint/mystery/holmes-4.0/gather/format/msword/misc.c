 ...
 */
BOOL
bReadBytes(UCHAR *aucBytes, size_t tMemb, ULONG ulOffset, struct fastbuf *pFile)
{
	fail(aucBytes == NULL || pFile == NULL);
	if( ulOffset >= (ULONG)LONG_MAX || ulOffset >= (ULONG)bfilesize(pFile))
          gerror(2200, "MSWord: Reference outside document");
	bseek(pFile, (long)ulOffset, SEEK_SET);

	if (bread(pFile, aucBytes, sizeof(UCHAR) * tMemb) != tMemb) {
		return FALSE;
	}
	return TRUE;
} /* end of bReadBytes */
 ...
 */
BOOL
bReadBuffer(struct fastbuf *pFile, ULONG ulStartBlock,
