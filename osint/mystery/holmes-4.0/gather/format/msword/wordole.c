...
#define PPS_ENTRY_TAG_SZNAME_SIZE 32
...
typedef struct pps_entry_tag {
	...
	ULONG	ulSize;
	int	iLevel;
	char	szName[PPS_ENTRY_TAG_SZNAME_SIZE];
	UCHAR	ucType;
} pps_entry_type;
 ...
 */
static ULONG
ulReadLong(struct fastbuf *pFile, ULONG ulOffset)
{
	UCHAR	aucBytes[4];
...
vName2String(char *szName, const UCHAR *aucBytes, size_t tNameSize)
{
	...
	if (tNameSize < 2
	    || tNameSize >= PPS_ENTRY_TAG_SZNAME_SIZE) { // XXX: silently ignore too long properties
		szName[0] = '\0';
		return;
	}
	for (tIndex = 0, pcChar = szName;
	     tIndex < 2 * tNameSize && aucBytes[tIndex];
	     tIndex += 2, pcChar++) {
 ...
 */
static size_t
tReadBlockIndices(struct fastbuf *pFile, ULONG *aulBlockDepot,
 ...
 */
static BOOL
bGetBBD(struct fastbuf *pFile, const ULONG *aulDepot, size_t tDepotLen,
 ...
 */
static BOOL
bGetSBD(struct fastbuf *pFile, const ULONG *aulDepot, size_t tDepotLen,
 ...
 */
static void
vGetBbdList(struct fastbuf *pFile, int iNbr, ULONG *aulBbdList, ULONG ulOffset)
{
	int	iIndex;
 ...
 */
static BOOL
bGetDocumentText(struct fastbuf *pFile, const pps_info_type *pPPS,
 ...
 */
static void
vGetDocumentData(struct fastbuf *pFile, const pps_info_type *pPPS,
	const ULONG *aulBBD, size_t tBBDLen,
	const UCHAR *aucHeader, int iWordVersion)
{
 ...
 */
int
iInitDocumentOLE(struct fastbuf *pFile, long lFilesize)
{
...
/* Small block list */
	...
	if(usIdent != 0x8098 &&	        /* Word 7 for oriental languages */
	  ...
	  gerror(2203, "MSWord: Unsupported document format");
