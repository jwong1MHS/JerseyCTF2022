 ...
 */
static void
vAnalyseSummaryInfo(const UCHAR *aucBuffer, ULONG size)
{
	ULONG	ulOffset;
	...
	for (tIndex = 0; tIndex < tCount; tIndex++) {
		...
		if (ulOffset >= size)
                        return; // summary info is broken, give it up
		NO_DBG_DEC(tPropID);
		NO_DBG_HEX(ulOffset);
 ...
 */
static void
vAnalyseDocumentSummaryInfo(const UCHAR *aucBuffer, ULONG size)
{
	ULONG	ulOffset;
	size_t	tIndex, tCount, tPropID, tPropType;
 ...
 */
static UCHAR *
pucAnalyseSummaryInfoHeader(struct fastbuf *pFile,
	...
	const ULONG *aulSBD, size_t tSBDLen)
{
	...
	DBG_DEC_C(ulOffset != P_HEADER_SZ + P_SECTIONLIST_SZ &&
		...
		ulOffset);
	if (unlikely(ulOffset != P_HEADER_SZ + P_SECTIONLIST_SZ &&
		ulOffset != P_HEADER_SZ + 2 * P_SECTIONLIST_SZ))
		werr(1, "Data corrupted");
	tLength =
		(size_t)ulGetLong(tSectionCount * P_SECTIONLIST_SZ, aucSecLst);
	NO_DBG_HEX(tLength);
	if(ulOffset + tLength > ulSize)
	  return NULL;
	if (unlikely(ulOffset + tLength > ulSize))
		werr(1, "Data corrupted");
 ...
 */
void
vSet0SummaryInfo(struct fastbuf *pFile, const UCHAR *aucHeader)
{
	UCHAR	*aucBuffer;
 ...
 */
void
vSet2SummaryInfo(struct fastbuf *pFile, int iWordVersion, const UCHAR *aucHeader)
{
	UCHAR	*aucBuffer;
 ...
 */
static void
vSetSummaryInfoOLE(struct fastbuf *pFile, const pps_info_type *pPPS,
	const ULONG *aulBBD, size_t tBBDLen,
	const ULONG *aulSBD, size_t tSBDLen)
{
	UCHAR	*pucBuffer;
	...
	if (pucBuffer != NULL) {
		vAnalyseSummaryInfo(pucBuffer, pPPS->tSummaryInfo.ulSize);
	...
	pucBuffer = pucAnalyseSummaryInfoHeader(pFile,
		...
		aulBBD, tBBDLen, aulSBD, tSBDLen);
	if (pucBuffer != NULL) {
		vAnalyseDocumentSummaryInfo(pucBuffer, pPPS->tDocSummaryInfo.ulSize);
		pucBuffer = xfree(pucBuffer);
	}
 ...
 */
void
vSet6SummaryInfo(struct fastbuf *pFile, const pps_info_type *pPPS,
	const ULONG *aulBBD, size_t tBBDLen,
	const ULONG *aulSBD, size_t tSBDLen,
 ...
 */
void
vSet8SummaryInfo(struct fastbuf *pFile, const pps_info_type *pPPS,
	const ULONG *aulBBD, size_t tBBDLen,
	const ULONG *aulSBD, size_t tSBDLen,
	const UCHAR *aucHeader)
{
