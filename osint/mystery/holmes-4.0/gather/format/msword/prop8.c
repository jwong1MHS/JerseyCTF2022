 ...
 */
void
vGet8SepInfo(struct fastbuf *pFile, const pps_info_type *pPPS,
...
#if 1	/* XXX: HACK fixing possible overflow and parser crash */
			iPosPrev = (int)usGetWord(
					iFodo + iFodoOff + 5,
					aucGrpprl);
			for (iIndex = 0; iIndex < iCol; iIndex++) {
				iPosCurr = (int)usGetWord(
					iFodo + iFodoOff + 7 + iIndex * 2,
					aucGrpprl);
				if (iPosCurr < iPosPrev || iPosCurr - iPosPrev > SHRT_MAX)
					gerror(2200, "MSWord: Corrupted document");
 ...
 */
void
vGet8LstInfo(struct fastbuf *pFile, const pps_info_type *pPPS,
	const ULONG *aulBBD, size_t tBBDLen,
	const ULONG *aulSBD, size_t tSBDLen,
	const UCHAR *aucHeader)
{
 ...
 */
void
vGet8PapInfo(struct fastbuf *pFile, const pps_info_type *pPPS,
	const ULONG *aulBBD, size_t tBBDLen,
	const ULONG *aulSBD, size_t tSBDLen,
	const UCHAR *aucHeader)
{
	...
	for (iIndex = 0; iIndex < (int)tLen; iIndex++) {
		if (aulParfPage[iIndex] > ULONG_MAX / BIG_BLOCK_SIZE)
			werr(1, "Paragraph info corrupted");
		if (!bReadBuffer(pFile, pPPS->tWordDocument.ulSB,
				aulBBD, tBBDLen, BIG_BLOCK_SIZE,
				aucFpage,
 ...
 */
void
vGet8ChrInfo(struct fastbuf *pFile, const pps_info_type *pPPS,
	const ULONG *aulBBD, size_t tBBDLen,
	const ULONG *aulSBD, size_t tSBDLen,
	const UCHAR *aucHeader)
{
