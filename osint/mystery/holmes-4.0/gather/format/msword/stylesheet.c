 ...
 */
void
vGet2Stylesheet(struct fastbuf *pFile, int iWordVersion, const UCHAR *aucHeader)
{
	...
	if(2 + tName + tChpx + tPapx + 2 + 2 * tStdCount > tStshInfoLen)
 ...
 */
void
vGet6Stylesheet(struct fastbuf *pFile, ULONG ulStartBlock,
 ...
 */
void
vGet8Stylesheet(struct fastbuf *pFile, const pps_info_type *pPPS,
	...
	const UCHAR *aucHeader)
{
	...
	do {
		...
		for (iIndex = 0, tOffset = 2 + (size_t)usGetWord(0, aucBuffer);
			...
			tPos = 2 + tStdBaseInFile;
			NO_DBG_DEC(tPos);
			if ((u64)tOffset + tPos > tStshInfoLen - 2)
			  werr(1, "Data corrupted");
