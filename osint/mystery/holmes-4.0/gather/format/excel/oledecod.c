...
int __OLEdecode (struct fastbuf *source, pps_entry ** stream_list, U32 * root,
...
/*  sbfilename[0] = 0; */
  ...
  *stream_list = NULL;
  ...
  test_exitf ((c = bgetc (source)) >= 0, 5, ends ());
  bungetc(source);
/* test_exitf (!isprint (c), 8, ends ()); OpenBSD suggestion to comment this out */
  ...
  U32 BDepot_size = 0x0200 * (num_bbd_blocks + num_xbbd_blocks);
  BDepot = (U8 *) xmalloc(BDepot_size);
  ...
  *_BDepot = BDepot;
  ...
  for (i = 0; i < MIN (num_bbd_blocks, 0x0200 / 4 - 19); i++)
    {
      ...
      assert (FilePos >= 0);
      excel_bseek (source, FilePos, SEEK_SET);
      test_exitf(bread (source, s, 0x0200) == 0x0200, 5, ends ());
      s += 0x0200;
    }

  Blockx = (U8 *) xmalloc(0x0200);
  ...
  for (i = 0; i < num_xbbd_blocks; i++)
  {
	 ...
	 assert (FilePos >= 0);
      excel_bseek (source, FilePos, SEEK_SET);
      test_exitf(bread (source, Blockx, 0x0200) == 0x0200, 5, ends ());
  ...
  sbd_list = (U32 *) xmalloc(ENTRYCHUNK * 4);
  ...
  for (len = 1; sbd_list[len - 1] != 0xfffffffeUL; len++)
	...
	{
	  U32 *newspace;
	  newspace = xrealloc(sbd_list,
			      (1 + len / ENTRYCHUNK) * ENTRYCHUNK * 4);
	  test_exitf (newspace != NULL, 10, ends ());
	  sbd_list = newspace;
	}
      if((sbd_list[len - 1] * 4) >= BDepot_size)
        gerror(2203, "Corrupted EXCEL file");
  ...
  *_SDepot = SDepot;
  ...
  root_list = (U32 *) xmalloc(ENTRYCHUNK * 4);
  ...
  for (len = 1; root_list[len - 1] != 0xfffffffeUL; len++)
	...
	{
	  U32 *newspace;
	  newspace = xrealloc(root_list,
			      (1 + len / ENTRYCHUNK) * ENTRYCHUNK * 4);
	  test_exitf (newspace != NULL, 10, ends ());
	  root_list = newspace;
	}
      if((root_list[len - 1] * 4) >= BDepot_size)
        gerror(2203, "Corrupted EXCEL file");
  ...
  *stream_list = pps_list = (pps_entry *)xmalloc(num_of_pps*sizeof(pps_entry));
...
/* FIXME MARK 2 */
	...
	/* root entry, sbfile must be generated */
	{
		...
		*_sbfilename = (U8 *)xmalloc(TMPNAM_LEN);
	...
	while (pps_start != 0xfffffffeUL)
	{
	    ...
	    excel_bseek (infile, FilePos, SEEK_SET);
            test_exitf(bread (infile, Block, bytes_to_read) == bytes_to_read, 5, ends ());
...
/* closeOLEtreefiles --- outdated because not to generate the
   real files by now --- cole 2.0.0 */
/*
#define freeNoNULL(x) { if ((x) != NULL) xfree(x); }
