...
cole_perror (const char *s, COLERRNO colerrno)
{
	...
	case COLE_EERRNO:
		...
		break;
	case COLE_EOPENFILE:
		gerror(2203, "cole - Cannot open the file");
		break;
	case COLE_ENOFILESYSTEM:
		gerror(2203, "cole - The file is not a OLE2 file");
		break;
	case COLE_EINVALIDFILESYSTEM:
		gerror(2203, "cole - The file has a short OLE2 header or it is not really an OLE2 file.");
		break;
	case COLE_EISNOTDIR:
		gerror(2203, "cole - The OLE2 entry is not a substorage object");
		break;
	case COLE_EISNOTFILE:
		gerror(2203, "cole - The substorage object is not valid");
		break;
	case COLE_EFILENOTFOUND:
		gerror(2203, "cole - OLE2 object not found");
		break;
	case COLE_EOF:
		gerror(2203, "cole - End of stream object has been reached");
		break;
	case COLE_EMEMBERISNOTDIR:
		gerror(2203, "cole - The OLE2 object searched for is not "
			 "a substorage object");
		break;
	case COLE_EBROKENFILENAME:
		gerror(2203, "cole - Illegal OLE object name");
		break;
	case COLE_EFILENAMEISNOTFILE:
		gerror(2203, "cole - The OLE2 object is not a stream");
		break;
	case COLE_EFSEEKDELTA:
		gerror(2203, "cole - The seek offset is an illegal value");
		break;
	case COLE_EFSEEKFLAG:
		gerror(2203, "cole - The Seek Flag is not valid");
		break;
	case COLE_EREAD:
		gerror(2203, "cole - Short read length returned...the file is probably corrupted");
		break;
	case COLE_EUNKNOWN:
		gerror(2203, "cole - An unknown error ocurred in the cole libary (might be a bug)");
		break;
	default:
		gerror(2203, "cole - An unknown error %d ocurred in the cole libabry (might be a bug)",
			 colerrno);
		break;
	}
}
 ...
 */
COLEFS *
cole_mount (struct fastbuf *source, COLERRNO *colerrno)
{
	COLEFS * ret;

	ret = (COLEFS *)xmalloc(sizeof (COLEFS));
	if (ret == NULL) {
        ...
        ret->file= source;
	switch (__OLEdecode (source, &ret->tree, &ret->root, &ret->BDepot,
...
cole_umount (COLEFS *colefilesystem, COLERRNO *colerrno)
{
	...
	ret = 0;
	xfree(colefilesystem->BDepot);
	xfree(colefilesystem->tree);
	bclose (colefilesystem->file);
	if (colefilesystem->SDepot != NULL) {
		xfree(colefilesystem->SDepot);
...
#endif
		xfree(colefilesystem->sbfilename);
	}
	xfree(colefilesystem);
...
static int
__cole_print_tree_indir(COLEDIR *cd UNUSED, void *info, COLERRNO *colerrno UNUSED)
{
/*
...
static int
__cole_print_tree_outdir(COLEDIR *cd UNUSED, void *info, COLERRNO *colerrno UNUSED)
{
/*
 * ATTENTION: if you modify this function so it modifies colerrno:
 * 	Modify colerrno comment in the functions that call it,
...
static int
__cole_print_tree_inroot(COLEDIR *cd, void *info UNUSED, COLERRNO *colerrno UNUSED)
{
/*
 * ATTENTION: if you modify this function so it modifies colerrno:
 * 	Modify colerrno comment in the functions that call it,
 ...
 */
	...
	printf ("DIR ");
	printf (" %7u", (int)cole_dir_getsize (cd));
...
static int
__cole_print_tree_indirentry(COLEDIRENT *cde, void *info, COLERRNO *colerrno UNUSED)
{
/*
 * ATTENTION: if you modify this function so it modifies colerrno:
 * 	Modify colerrno comment in the functions that call it,
 ...
 */
	...
	if (cole_direntry_isdir (cde))
		...
		printf ("????");
	printf (" %7u", (int)cole_direntry_getsize (cde));
	printf (" %08lx-%08lx %08lx-%08lx",
...
cole_opendir_rootdir (COLEFS *colefilesystem, COLERRNO *colerrno)
{
	...
	ret = (COLEDIR *)xmalloc(sizeof (COLEDIR));
	if (ret == NULL) {
		if (colerrno != NULL) *colerrno = COLE_EMEMORY;
		return NULL;
	}
 ...
 */
int
cole_closedir (COLEDIR *coledir, COLERRNO *colerrno UNUSED)
{
	xfree(coledir);
...
cole_fopen_direntry (COLEDIRENT *coledirentry, COLERRNO *colerrno)
{
	...
	ret = (COLEFILE *)xmalloc(sizeof (COLEFILE));
	if (ret == NULL) {
		if (colerrno != NULL) *colerrno = COLE_EMEMORY;
		return NULL;
	}
...
static int
__cole_locate_filename_visitdir (COLEDIR *cd UNUSED, void *info)
{
