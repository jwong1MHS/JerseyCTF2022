/*
 *  Porter's Stemming Algorithm for English as described in:
 *
 *    Martin Porter: An algorithm for suffix stripping, 1980, Program, Vol. 14, no. 3, pp 130-137.
 *
 *  Implementation by the author (see http://www.tartarus.org/~martin/PorterStemmer)
 *  with a couple of improvements against the version published in the paper
 *  (these are marked with DEPARTURE), adapted for Sherlock by Martin Mares.
 *  I believe the original code was public domain and so is my version.
 *
 *  This stemmer supports only conversion of a word to its stem.
 */
...
/* Globals: b is a buffer holding a word to be stemmed. The letters are in b[k0],
   b[k0+1] ... b[k]. k is readjusted as the stemming progresses.
*/
...
clist *
stem_porter(struct stemmer *st UNUSED, struct word_node *w, struct mempool *mp)
{
  if (w->word_form != WORD_FORM_OTHER || w->stem_form != WORD_FORM_LEMMA)
    return NULL;			/* Operation not supported */

  byte *src = w->w;
  byte dest[strlen(src)+1];
  byte *d = dest;
  ...
  *d = 0;
  ...
  clist_init(res);
  word_list_add(mp, res, dest, WORD_FORM_OTHER, WORD_FORM_STEM, 0, 0);
  return res;
}

void
stem_init_porter(struct stemmer *st UNUSED)
{
}
